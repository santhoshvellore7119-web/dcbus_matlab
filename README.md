...
An AI/ML-driven continuous voltage controller designed to replace traditional Proportional-Integral (PI) control in DC microgrid / DC bus systems using **Deep Deterministic Policy Gradient (DDPG)**, grounded in the case study dataset `Case Study DCbusData.csv (1).xlsx`.

---

## 📌 Project Objectives

1. **Replace Traditional PI Control:** Formulate DC bus voltage stabilization as a continuous Deep Reinforcement Learning (DRL) control problem (no conventional fuzzy logic or manual heuristic tuning).
...
```
  V* (300V Ref) ──(+)──┐
                       ├──> V_err ───> [ DDPG Actor Network ] ───> Control Effort (u) ───> [ DC Bus Converter ]
  V (Sensed)    ──(-)──┘                                                                          │
                                                                                                  ▼
...
| Performance Metric | Historical PI Controller (Data) | Trained DRL Controller (DDPG) | Winner / Improvement |
| :--- | :---: | :---: | :---: |
...
├── train_ddpg_dcbus.m              # DDPG Agent Architecture & Training Hyperparameters
...
### 1. Evaluate Pre-Trained Model (Instant 1-Click)
